...
public class Repositories_TK {
    ...
    public ArrayList<Model.Model_TK> getALl_THD1T() {
        ArrayList<Model.Model_TK> List_THD1N = new ArrayList<>();
        sql = "SELECT COUNT(MAHD) FROM HOADON WHERE MONTH(NgayThanhToan) = MONTH(GETDATE()) AND YEAR(NgayThanhToan) = YEAR(GETDATE())";
        try {
            con = dbconnect.DBconnect.getConnection();
    ...
    public ArrayList<Model.Model_TK> getALl_TDT1N() {
        ...
        sql = "SELECT SUM(TongTien) FROM HOADON WHERE CAST(NgayThanhToan AS DATE) = CAST(GETDATE() AS DATE)";
        try {
            con = dbconnect.DBconnect.getConnection();
            ps = con.prepareCall(sql);
            rs = ps.executeQuery();
    ...
    public ArrayList<Model.Model_TK> getALl_TDT1T() {
        ...
        sql = "SELECT SUM(TongTien) FROM HOADON WHERE MONTH(NgayThanhToan) = MONTH(GETDATE()) AND YEAR(NgayThanhToan) = YEAR(GETDATE())";
        try {
            con = dbconnect.DBconnect.getConnection();
            ps = con.prepareCall(sql);
            rs = ps.executeQuery();
